...
if psql -U listmonk -d listmonk -h localhost -c "SELECT 1;" > /dev/null 2>&1; then
    ...
    echo "1. Éditez /etc/postgresql/*/main/pg_hba.conf"
...
if psql -U listmonk -d listmonk -h localhost -c "SELECT COUNT(*) FROM departement_region_mapping;" > /dev/null 2>&1; then
...
echo "   • Tester DB: psql -U listmonk -d listmonk -h localhost"
echo ""
