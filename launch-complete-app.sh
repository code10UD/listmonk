...
# Script de lancement complet et simplifié de Listmonk avec interface géographique
...
./listmonk --install --config config-complete.toml || {
    echo "⚠️  Installation déjà effectuée, continuons..."
}
...
./listmonk --config config-complete.toml &
BACKEND_PID=$!
...
echo "   Base de données : PostgreSQL avec données géographiques"
echo ""
...
echo "   ✅ Backend Listmonk avec interface intégrée"
echo ""
echo "📋 TESTS À EFFECTUER :"
echo "   1. Aller sur http://localhost:9000/admin"
echo "   2. Naviguer vers 'Abonnés'"
echo "   3. Cliquer sur 'Recherche avancée'"
